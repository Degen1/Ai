...
ሳራ is an Expo/React Native chat app with a separate OpenAI-backed server. Text and photo messages use the Responses API. The API key stays on the server and is never bundled into the mobile app.
...
`GET /health` reports whether the server has a key, without revealing it. `OPENAI_MODEL` can change the model; it defaults to `gpt-6-luna`.
